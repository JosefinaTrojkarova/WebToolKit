...
const alternatives = ref<Alternatives[]>([])

watch(data, async (newData) => {
    if (newData && newData.alternatives.length > 0) {
        const { data: altData } = await useFetch<Alternatives[]>('/api/tool/alternatives', {
            method: 'POST',
            body: { ids: newData.alternatives }
        })
