...
onMounted(() => {
  ...
  const toolCards = [...filteredTools.value].map((data) => {
...
<template>
  <NuxtLayout :footerDisabled="true">
    <main>
      <header>
        <h1>Oops...</h1>
        <p class="message">{{ error.statusCode === 404 ? 'Page not found' : 'An error occured on our side' }}</p>
      </header>
      <div class="buttons">
        <NuxtLink to="/" class="btn--primary--large">Go back home</NuxtLink>
        <p>or</p>
